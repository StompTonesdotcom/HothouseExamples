...
void AudioCallback(AudioHandle::InputBuffer in, AudioHandle::OutputBuffer out,
  ...
  filterKnob =
      hw.knobs[Hothouse::KNOB_3].Process();  // Knob 3: Lowpass/Highpass
  flutterAmount = hw.knobs[Hothouse::KNOB_4].Process();  // Knob 4: Flutter
  saturationAmount =
      hw.knobs[Hothouse::KNOB_5].Process();            // Knob 5: Saturation
  ...
  // *** Commented out due to compiler warning ***
  // `unused variable 'filterBlend' [-Wunused-variable]`
  ...
  if (filterKnob < (0.5f - deadZone)) {
    ...
    float curved =
        normalized * normalized;  // Square for more low-freq emphasis
  ...
  if (filterKnob > (0.5f + deadZone)) {
    ...
    float normalized =
        (filterKnob - 0.5f - deadZone) / (0.5f - deadZone);  // 0.0-1.0
  ...
  for (size_t i = 0; i < size; ++i) {
    if (bypass) {
      ...
      // Process wow and flutter (pitch modulation) - always process to avoid
      // clicks Calculate combined pitch modulation with nested modulation for
      // wow
      ...
      smoothedPitchShift = smoothedPitchShift * smoothingCoeff +
                           targetPitchShift * (1.0f - smoothingCoeff);
